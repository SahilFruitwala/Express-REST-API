...
class Feed extends Component {
  ...
  componentDidMount() {
    ...
    fetch("http://localhost:3333/graphql", {
      ...
      .then((resData) => {
        console.log(resData);
        if (resData.errors) {
          throw new Error("Fertching failed!");
        }
  ...
  finishEditHandler = (postData) => {
    ...
    fetch("http://localhost:3333/post-image", {
      method: "PUT",
      headers: {
        Authorization: "Bearer " + this.props.token,
      },
      ...
      .then((data) => {
        ...
        if (data.filePath) {
          imageUrl = data.filePath.split("\\")[1];
        }
        let graphqlQuery1 = {
          query: `
        ...
        console.log(graphqlQuery1);

        return fetch("http://localhost:3333/graphql", {
          method: "POST",
          body: JSON.stringify(graphqlQuery1),
          headers: {
            Authorization: "Bearer " + this.props.token,
            "Content-Type": "application/json",
          },
